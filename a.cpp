...
string chitiethoadon[6] = {"1. Nhập mã vật tư(VD: VT0099):","2. Nhập số lượng", "3. Nhập đơn giá:","4. Nhập VAT:"," Hủy "," Xác nhận "};
void Normal () {
	SetColor(15); // white 
	SetBGColor(0);// black
}
    ...
    do
  {
    ...
    do{
        ...
        switch (luachon) {
            ...
            getline(cin, X.vattu.MAVT);break;
...
void display(PTRHD Dshd, int v, int n){//duyệt danh sách 
     ...
     if (Dshd == NULL){ 
      ...
      int cot = 0;
        ...
        gotoxy(8, 6+cot);
            cout << a1[i]->hd.SoHD; gotoxy(16,6+cot);cout<<a1[i]->hd.Loai;gotoxy(39,6+cot);
            cout<< a1[i]->hd.NgayLapHoaDon.day<<"/"<<a1[i]->hd.NgayLapHoaDon.month<<"/"<<a1[i]->hd.NgayLapHoaDon.year;
         cot++; 
    }
}
